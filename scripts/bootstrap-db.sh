...
echo "==> Looking up bastion + RDS for ${PROJECT}-${ENV}..."

BASTION_ID=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=${PROJECT}-${ENV}-bastion" \
            "Name=instance-state-name,Values=running" \
  --region "$REGION" \
  --query 'Reservations[0].Instances[0].InstanceId' --output text)

if [[ -z "$BASTION_ID" || "$BASTION_ID" == "None" ]]; then
  echo "ERROR: no running bastion found for ${PROJECT}-${ENV}-bastion" >&2
  exit 1
fi
